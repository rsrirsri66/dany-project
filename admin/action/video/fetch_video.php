...
while ($row = mysqli_fetch_assoc($result)) {
    $data[] = [
        ...
        "id"        => $row['video_id'],
        "action"    => '
            <button class="btn btn-sm text-warning" onclick="goEdit(' . $row['video_id'] . ');" data-bs-toggle="modal" data-bs-target="#editFoodModal"><i class="lni lni-pencil"></i></button>
            <button class="btn btn-sm text-danger" onclick="goDeleteEmployee(' . $row['video_id'] . ');"><i class="lni lni-trash"></i></button>
            <button class="btn btn-sm text-info" onclick="toggleStatus(' . $row['video_id'] . ', \'' . $row['video_status'] . '\')">
